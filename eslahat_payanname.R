...
boursenews_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_boursenews_ir (2).csv",encoding = "UTF-8")
deghtesad_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_deghtesad.csv",encoding = "UTF-8")
FarsNews_Agency_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_FarsNews_Agency.csv",encoding = "UTF-8")
NavadeEghtesadi_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_NavadeEghtesadi.csv",encoding = "UTF-8")
SharghDaily_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_SharghDaily.csv",encoding = "UTF-8")
Tasnimnews_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_Tasnimnews_Fa (1).csv",encoding = "UTF-8")
tejaratnews_data<-read.csv("C:\\Users\\98939\\Desktop\\data_twitter_new\\_tejaratnews.csv",encoding = "UTF-8")


boursenews<-boursenews_data$tweet
deghtesad<-deghtesad_data$tweet
FarsNews_Agency<-FarsNews_Agency_data$tweet
NavadeEghtesadi<-NavadeEghtesadi_data$tweet
SharghDaily<-SharghDaily_data$tweet
Tasnimnews<-Tasnimnews_data$tweet
tejaratnews<-tejaratnews_data$tweet


##############################################################################
lex<-read.csv("C:\\Users\\98939\\Desktop\\text mining\\final_lexicon_without_duplicated_values_and_zeros.csv",encoding = "UTF-8")
#lex2<-read.csv("C:\\Users\\98939\\Desktop\\text mining\\LexiPers.csv",encoding = "UTF-8")
...
#positive.words2<-filter(lex2 ,Label==1)$Name
#negative.words2<-filter(lex2 ,Label==-1)$Name
...
# positive.words.bl <- unique(c(positive.words2,positive.words,positive.words.bl))
# negative.words.bl  <- unique(c(negative.words2,negative.words,negative.words.bl))
...
sentment_analisiser<-function(tweet,data){
...
sentment_boursenews<-sentment_analisiser(boursenews,boursenews_data)$data_final
sentment_deghtesad<-sentment_analisiser(deghtesad,deghtesad_data)$data_final
sentment_FarsNews_Agency<-sentment_analisiser(FarsNews_Agency,FarsNews_Agency_data)$data_final
sentment_NavadeEghtesadi<-sentment_analisiser(NavadeEghtesadi,NavadeEghtesadi_data)$data_final
sentment_SharghDaily<-sentment_analisiser(SharghDaily,SharghDaily_data)$data_final
sentment_Tasnimnews<-sentment_analisiser(Tasnimnews,Tasnimnews_data)$data_final
sentment_tejaratnews<-sentment_analisiser(tejaratnews,tejaratnews_data)$data_final


empty_date<-function(inputdata){

start = inputdata$date[1]
end=inputdata$date[length(inputdata$date)]

#length(seq(from=start, to=end, by='day')) - 1

full <- seq(from=start, to=end, by='day')

data_final_new<-data.frame(Date=full,sentiment=with(inputdata, inputdata$sentiment[match(full,inputdata[,1])]))
...
sentment_boursenews<-empty_date(sentment_boursenews)
sentment_deghtesad<-empty_date(sentment_deghtesad)
sentment_FarsNews_Agency<-empty_date(sentment_FarsNews_Agency)
sentment_NavadeEghtesadi<-empty_date(sentment_NavadeEghtesadi)
sentment_SharghDaily<-empty_date(sentment_SharghDaily)
sentment_Tasnimnews<-empty_date(sentment_Tasnimnews)
sentment_tejaratnews<-empty_date(sentment_tejaratnews)


colnames(sentment_boursenews)<-c("Date","sentiment_BourseNews")
colnames(sentment_deghtesad)<-c("Date","sentiment_donyaie_eghtesad")
colnames(sentment_FarsNews_Agency)<-c("Date","sentiment_FarsNews")
colnames(sentment_NavadeEghtesadi)<-c("Date","sentiment_navad_eghtesadi")
colnames(sentment_SharghDaily)<-c("Date","sentiment_Shargh")
colnames(sentment_Tasnimnews)<-c("Date","sentiment_TasnimNews")
colnames(sentment_tejaratnews)<-c("Date","sentiment_TejaratNews")




# #put all data frames into list
# twitter_list <- list(sentment_boursenews,sentment_deghtesad,sentment_FarsNews_Agency,sentment_NavadeEghtesadi,sentment_SharghDaily,sentment_Tasnimnews,sentment_tejaratnews)
# 
# #merge all data frames in list
# data_twitter<-Reduce(function(x, y) merge(x, y, by= 'Date'), twitter_list)
# 
# colnames(data_twitter)<-c("Date","sentment_boursenews","sentment_deghtesad","sentment_FarsNews_Agency","sentment_NavadeEghtesadi","sentment_SharghDaily","sentment_Tasnimnews","sentment_tejaratnews")
###############################################################################################################
...
twitter_list <- list(data_asli,sentment_boursenews,sentment_deghtesad,sentment_FarsNews_Agency,sentment_NavadeEghtesadi,sentment_SharghDaily,sentment_Tasnimnews,sentment_tejaratnews,trend_data)
...
inclusionprobs <- reshape2::melt(summary(bsts.reg.priors,burn=burn)$coefficients[,"inc.prob"][summary(bsts.reg.priors,burn=burn)$coefficients[,"inc.prob"]>0.01])
inclusionprobs$Variable <- as.character(row.names(inclusionprobs))
ggplot(data=inclusionprobs, aes(x=Variable, y=value)) + 
  geom_bar(stat="identity", position="identity") + 
  theme(axis.text.x=element_text(angle = -90, hjust = 0)) + 
...
plot(1:311 ,scale(data_train$adjClose) ,type="l",col="blue",lwd = 3)
par(new=TRUE)
plot(1:311,data_train$`?????? ????????`,type="l")
...
model %>% fit(X,y, epochs=50, batch_size=16, shuffle = FALSE)
...
prior <-SpikeSlabPrior(x=model.matrix(adjClose~., data=as.ts(data_train[,1:8]) ), 
                       ...
                       #prior.df = .01,           # weaker prior than the default
                       prior.inclusion.probabilities=rep(0.3,ncol(data_train[,1:8])),
                       diagonal.shrinkage = 0# use Zellner's prior
)
...
#ss <- AddMonthlyAnnualCycle(ss,data_train)
ss <-AddLocalLinearTrend(ss,as.ts(data_train$adjClose) )
ss <- AddSeasonal(ss, as.ts(data_train$adjClose),nseasons =52)
...
model %>% fit(X,y, epochs=200, batch_size=16, shuffle = FALSE,verbose=0)
y_pred = model %>% predict(X)
...
dlstm<- data.frame(
  # fitted values and predictions
  # actual data and dates 
  as.numeric(c(coredata(data_train),forcasted_T)),
  as.numeric(coredata(data$adjClose)),
  c(time(data_train),forcasted_date)
)
...
MAPE4<-MAPE(forcasted_T,coredata(data_test))
RMSE4<-RMSE(forcasted_T,coredata(data_test))
MAE4<-MAE(forcasted_T,coredata(data_test))
####################################################################
library(plotly)
